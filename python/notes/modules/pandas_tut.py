...
# df.loc["id_1"]       # Row by label

# df["A"]              # Column by label

# df.loc["id_1"]["A"]  # Individual Cells
# df.loc[:,["A","B"]]  # Individual Cells , Slicing
# df.loc[:,:]          # Individual Cells , Slicing

# df   = df[["A","B"]] # Select column subset

# df.iloc[n:]           # Drop first n rows
# df.drop(df.index[:n]) # Drop first n rows
# df.tail(-n)           # Drop first n rows


# df.append(pandas.Series(name='NameOfNewRow')) # Add new empty row
# df.append(pandas.Series(), ignore_index=True) # Add new empty row , no name
...
# print ( (df > 5).astype(int) )               # Binary Mask
...
def df_read_relative(path):
    ...
    # html wikipedia return a list of dataframes
    # pandas creates dfs from all html tables it finds on the page
    # you have to manually figure out which one you meant and fill in the index
    # df = pd.read_html(url)[index_of_table]
